...
class InGamePosition {
  ...
  update(play) {
    ...
    for (let i = 0; i < this.ufos.length; i++) {
      let ufo = this.ufos[i];
      if (!frontRowUFOs[ufo.column] || frontRowUFOs[ufo.column].row < ufo.row) {
        frontRowUFOs[ufo.column] = ufo;
      }
    }
    // Give a chance for bombing
    for (let i = 0; i < this.setting.ufoColumns; i++) {
      let ufo = frontRowUFOs[i];
      if (!ufo)
        continue;
  ...
  keyDown(play, keyboardCode) {
    if (keyboardCode == 'KeyS') { // Mute sound: S
      play.sounds.muteSwitch();
    }
    if (keyboardCode == 'KeyP') { // Pause: P
